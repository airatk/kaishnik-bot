...
def buildings_dialer() -> str:
    ...
    for (number, building) in enumerate(BUILDINGS):
        buildings_dialer_keyboard.add_text_button(
...
def libraries_dialer() -> str:
    ...
    for (number, library) in enumerate(LIBRARIES):
        libraries_dialer_keyboard.add_text_button(
            text=library["button"] if number < 4 else library["title"], 
            payload={
...
def sportscomplex_dialer() -> str:
    ...
    for (number, sportscomplex_building) in enumerate(SPORTSCOMPLEX):
        sportscomplex_dialer_keyboard.add_text_button(
            text=sportscomplex_building["button"], 
            payload={
...
def dorms_dialer() -> str:
    ...
    for (number, dorm) in enumerate(DORMS):
        dorms_dialer_keyboard.add_text_button(
            text=dorm["button"], 
            payload={
